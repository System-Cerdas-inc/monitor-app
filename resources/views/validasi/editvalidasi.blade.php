    ...
    <div class="row">
        <div class="col-sm-12 col-lg-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('updatevalidasi', $validasis->id) }}" method="POST">
                        {{ method_field('PUT') }}
                        @csrf
                        <div class="row">
                                ...
                                <select class="form-select @error('site_id') is-invalid @enderror" name="site_id"
                                    ...
                                    @foreach ($sites as $site)
                                        <option value="{{ $site->id }}"
                                            {{ old('site_id', $validasis->site_id) == $site->id ? 'selected' : '' }}>
                                            {{ $site->domain }}
                                            {{ old('domain') }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6 mt-2">
                                ...
                                <select class="form-select @error('status') is-invalid @enderror" required
                                    ...
                                    @foreach ($status as $st)
                                        <option value="{{ $st }}"
                                            {{ old('status', $validasis->status) == $st ? 'selected' : '' }}>
                                            {{ $st }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-6 mt-2">
                                ...
                                <select class="form-select @error('status_validasi') is-invalid @enderror" required
                                    ...
                                    @foreach ($status_validasi as $val)
                                        <option value="{{ $val }}"
                                            {{ old('status_validasi', $validasis->status_validasi) == $val ? 'selected' : '' }}>
                                            {{ $val }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-12 mt-4">
                                ...
                                <div class="mt-2">
                                    <embed type="application/pdf" src="" width="700" height="350"></embed>
                            ...
                            <div class="col-md-12 mt-4">
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control @error('catatan') is-invalid @enderror" style="height: 200px" required name="catatan">{{ old('catatan', $validasis->catatan) }}</textarea>
                                </div>
                            </div>
                            <div class="form-group">
                                <button class="btn btn-primary float-end" type="submit">Submit</button>
                                <a href="{{ route('listvalidasi') }}" class="btn btn-warning float-first"
                                    type="button">Back</a>
                            </div>
                        </div>
                    </form>
